...
use Sokeio\Translatable\Traits\Relationship;
use Sokeio\Translatable\Traits\Scopes;
...
trait Translatable
{
    ...
    protected function getFallbackLocale(?string $locale = null): ?string
    {
        if (
            $locale &&
            Locale::isLocaleCountryBased($locale) &&
            $fallback = Locale::getLanguageFromCountryBasedLocale($locale)
        ) {
            return $fallback;
        }
